    ...
    <div id="page-content">
        ...
        <div class="block full">
        	<div class="block-title">
                <h2>Manage Items Images</h2>
                <div class="block-options pull-right">
    	    		 <div id="esearch" class="dataTables_filter">
    		    		<form class="form-wrap" method='get' action='/admin/items-images/{{$items->id}}'>
    	        ...
    	        <div class="dataTables_wrapper form-inline no-footer">
    		        <div class="row">
    			       <div class="col-xs-12">
    				        <div class="pull-right">
                                <a href="/admin/items-images/{{$items->id}}/create" class="btn btn-success"><i class="fa fa-plus"></i> Add Items Images</a>
                            </div>
    					</div>
    				</div>
    				@if($images->count())
                                        ...
                                        <tr @if($data->is_active == 0) class="danger" @endif >
                                            <td>
                                                @if(count($images) > 1)
                                                    ...
                                                    @if($a != 1)
                                                        <a class="btn btn-sm btn-info" href="/admin/items-images/{{$items->id}}/sort/up/{{$data['id']}}/{{$data['sort']}}/" ><i class="fa fa-caret-up"></i></a>
                                                    @endif

                                                    {{--<!--if its not the last item add down arrow-->--}}
                                                    @if($a != count($images))
                                                        <a class="btn btn-sm btn-info" href="/admin/items-images/{{$items->id}}/sort/down/{{$data['id']}}/{{$data['sort']}}/" ><i class="fa fa-caret-down"></i></a>
                                                    @endif
                                                @endif
                                            </td>
                                            ...
                                            <td class="text-left">
                                                <a href="/admin/items-images/{{$items->id}}/edit/{{$data->id}}/" data-toggle="tooltip" title="Edit Items Image" class="btn btn-effect-ripple btn-sm btn-success"><i class="fa fa-pencil"></i></a>
                                                <a href="/admin/items-images/{{$items->id}}/delete/{{$data->id}}/" data-toggle="tooltip" title="Delete Items Image" class="btn btn-effect-ripple btn-sm btn-danger"><i class="fa fa-times"></i></a>
                                            </td>
                                        </tr>
                                    <?php $a++; ?>
                                    @endforeach
                    ...
                    @else
    		             <div class="row no-result">
    			            <div class="col-xs-12">
    							<p>There are no Items Images to display.</p>
    			            </div>
    		            </div>
    			    @endif
